...
<jsp:useBean id="rolesInClaimOfferForm" scope="request"
             type="com.silvermoongroup.fsa.web.claimmanagement.form.RolesInClaimOfferForm"/>

<c:choose>
    <c:when test="${!empty rolesInClaimOfferForm.rolesInClaimOffer}">
        <table class="table table-striped table-hover table-condensed" id="roleTable">
            ...
            <c:forEach var="roleInClaimOffer" items="${rolesInClaimOfferForm.rolesInClaimOffer}">
                <tr>
    ...
    <c:otherwise>
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12 no-results"><fmt:message key="page.claimnavigator.label.noclaimofferroles" /></div>
            </div>
        </div>
    </c:otherwise>
