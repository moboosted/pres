...
<c:if test="${!empty structuredActualDTO.pendingRequests}">
	<table cellpadding="6px">
		<tr>
			...
			<td class="lightgrey"><fmt:message key="page.agreement.pendingrequests.heading" /></td>
		</tr>
	</table>
	<div id="raisedRequests">
		<table width="100%">
			...
			<c:forEach items="${structuredActualDTO.pendingRequests}" var="pendingRequestDTO">
				<tr class="odd">
					<td class="title" title="<smg:fmtSpecificationObjectHint value="${pendingRequestDTO}"/>">
					   <smg:fmtSpecificationObjectName value="${pendingRequestDTO}" /></td>
					<td class="title"><smg:fmt value="${pendingRequestDTO.requestedDate}" /></td>
					<td class="title">
                      <smg:fmt value="${pendingRequestDTO.state}" />
                      <c:if test="${! empty pendingRequestDTO.executedDate}">
                      (<smg:fmt value="${pendingRequestDTO.executedDate}" />)
                      </c:if>
                    </td>
                    <td class="title">
                      <c:url value="${contextPath}.do?method=.viewRequest" var="requestActionUrl">
                        <c:param name="requestObjectReference" value="${pendingRequestDTO.objectReference}" />
